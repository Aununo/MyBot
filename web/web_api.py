...
BILI_USER_AGENT = "Mozilla/5.0 (compatible; MyBot/1.0; +https://github.com)"
...
BILI_PROXY_FILE = DATA_DIR / "bili_proxy.json"
...
def build_bili_cookies() -> Dict[str, str]:
    cookies = {}
    if BILI_SESSDATA:
...
def read_proxy_cache() -> Dict[str, Dict]:
    if not BILI_PROXY_FILE.exists():
        return {}
    try:
        with open(BILI_PROXY_FILE, "r", encoding="utf-8") as f:
            return json.load(f)
    except Exception:
        return {}


def write_proxy_cache(data: Dict[str, Dict]) -> None:
    BILI_PROXY_FILE.parent.mkdir(parents=True, exist_ok=True)
    with open(BILI_PROXY_FILE, "w", encoding="utf-8") as f:
        json.dump(data, f, ensure_ascii=False, indent=2)


@app.get("/bili/proxy/{token}")
async def proxy_bili_video(token: str, request: Request):
    """
    B 站视频代理播放链接
    """
    cache = read_proxy_cache()
    ...
    if not entry or entry.get("expires_at", 0) <= now:
        ...
        raise HTTPException(status_code=404, detail="代理链接已失效")

    url = entry.get("url")
    if not url:
        raise HTTPException(status_code=404, detail="代理链接不存在")
    ...
    if resp.status_code >= 400:
        await resp.aclose()
        await client.aclose()
        raise HTTPException(status_code=resp.status_code, detail="上游资源不可用")

    upstream_headers = {}
    for key in ("content-type", "content-length", "content-range", "accept-ranges"):
        if key in resp.headers:
            upstream_headers[key] = resp.headers[key]
